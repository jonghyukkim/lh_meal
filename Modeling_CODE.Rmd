...
train_wj <- read_csv("train_wj.csv")
test <- read_csv("test_new.csv")
test_wj <- read_csv("test_wj.csv")

train <- train %>% 
  cbind(train_wj %>% select(New_lunch:Dinner_Shrimp))

test <- test %>% 
  cbind(test_wj %>% select(New_lunch:Dinner_Shrimp))
...
rf_cv_results %>% show_best() # mtry=11, CV mae=68.1
...
bayes_cv_results %>% show_best() # mae=66.4
bayes_cv_results %>% select_best("mae") # mtry=12, min_n=8, tree_depth=6


# Final lunch model
trainset_final <- train_recipe_xg %>% prep() %>% bake(training_set)
validset_final <- train_recipe_xg %>% prep() %>% bake(testing_set)
...
rf_cv_results %>% show_best() # mtry=12, CV mae=43.3
...
# Find best hyperparameter
bayes_cv_results %>% show_best() # mae=45.9
bayes_cv_results %>% select_best("mae") # mtry=12, min_n=9, tree_depth=11
